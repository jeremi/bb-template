...
- Event-driven APIs over brokered transports and event streams, documented in AsyncAPI 3 (MQTT, AMQP, Kafka, WebSockets, SSE).
...
flowchart TD
    ...
    Q -->|"Brokered channels or event streams<br/>(MQTT, AMQP, Kafka, WebSockets, SSE)"| A["AsyncAPI 3<br/>default api/asyncapi.yaml or api/index.yaml entry"]
...
The guide's candidate specification identifier is `govstack-cfr-api`, its current version is `0.2.0-draft`, and its proposed parent is the developing `govstack-cfr` specification. The proposed relationships are:
...
The guide uses RFC 2119 language: **MUST**, **MUST NOT**, **SHOULD**, **SHOULD NOT**, **MAY**. A MUST rule is mandatory for conformance. A SHOULD rule is expected by default and requires written justification to skip. A MAY rule is optional.

Each rule has exactly one level. The first paragraph of a rule body uses keywords from one family only (MUST and MUST NOT, SHOULD and SHOULD NOT, or MAY), and that family is the rule's level. Later paragraphs add weaker statements, examples, and carve-outs, never a stronger statement. The index builder rejects a rule that breaks this convention, so the `level` recorded for each rule in `rules.yaml` is derived from the page rather than asserted separately.

`rules.yaml` and the companion Spectral ruleset are the reference expressions of the rules: `rules.yaml` for each rule's identity, level, enforcement class, and API kinds, and the ruleset for the machine-checkable subset. The prose on each page supplies the rationale, examples, and carve-outs that give a rule its meaning. If the prose and an artifact disagree, that is a defect in the guide, resolved by an erratum rather than by preferring one locally. Each rule is tagged with an enforcement class ([§1.9](#19-rule-enforcement-classes)) that says whether a linter or a reviewer checks it, [§1.8](#18-layering-what-this-guide-constrains) says which layer it constrains, and [§1.11](#111-which-rules-apply-to-your-api) says which API kinds it applies to.
...
Where this guide adopts an external standard or convention on the OpenAPI 3.1, CloudEvents, or AsyncAPI 3 surface, that standard or convention takes precedence over the guide's generic rules for the fields or payloads it covers. The known precedences are:
...
3. **Behavioural-contract** rules state run-time behaviour an integrator relies on across BBs (for example, [§14.4](part-d/14-idempotency.md#144-replay-returns-original-response) idempotent replay, [§19.1](part-e/19-localisation.md#191-honour-the-request-language) honouring the request language). They are part of the interface contract but cannot be linted from the spec; they require implementation-level conformance testing. [Rules by API kind](rules-by-kind.md) lists them under the deployment profile so that a spec author can tell them apart from obligations on the specification document itself.
...
The GovStack governance process owns the transition schedule, conformance levels, and enforcement dates for existing BBs. Every canonical specification pins the exact guide and ruleset versions it uses under [§20.3](part-e/20-conformance-and-validation.md#203-declared-guide-conformance-version); validation tooling **MUST NOT** silently substitute a newer compatible-looking version. The intended cadence is at most one guide release per year, so that a specification pinned to a guide version stays on a current version for at least a year. A BB moves to a newer guide version when it publishes its next specification version, not mid-cycle.
...
## 1.11 Which rules apply to your API <a href="#111-which-rules-apply-to-your-api" id="111-which-rules-apply-to-your-api"></a>

Not every rule applies to every BB. Each rule in `rules.yaml` carries a `kinds` list drawn from four values, and [Rules by API kind](rules-by-kind.md) groups the rules by them:

- **read**: a read-only HTTP API, where `POST` appears only for search or lookup under [§6.6](part-b/6-http-methods.md#66-post-search-for-complex-queries). This is the smallest set and the one to start from.
- **write**: an HTTP API with mutations, in addition to the read rules: creation, status codes for writes, idempotency, asynchronous operations.
- **events**: an event or webhook surface: CloudEvents, AsyncAPI channels, subscription management.
- **deployment**: run-time obligations, such as replay semantics or language handling, assessed through implementation-level conformance tests. A rule that also requires a specification declaration carries the relevant API kinds as well; its declaration is assessed in the specification and its behaviour in the implementation.

A BB reads the kinds that match the surfaces it exposes and applies each rule where its stated conditions hold. For example, §14.2 applies when an HTTP API uses idempotency keys, including a read API that opts into them. A deployment-only rule has no specification declaration duty. The assignments live in `tools/rule-kinds.json`, as a default per section plus per-rule overrides, and the index builder checks them, so the page and `rules.yaml` cannot drift apart.
