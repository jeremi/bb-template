 ...
 * expose interfaces to create, list, and delete a subscription. This is a
 * proxy: it can only reason about paths that are
 * *named* as subscription resources. When a document exposes any subscription
 * path, the three baseline capabilities are required; a document with no subscription
...
export default function subscriptionEndpoints(targetVal, options, context) {
  ...
  return [
    {
      message: `subscription management is exposed but missing: ${missing.join(', ')}. §16.11 requires create, list, and delete interfaces.`,
      path: base,
    },
  ];
}
