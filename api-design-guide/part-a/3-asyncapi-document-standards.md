...
## 3.1 Qualified AsyncAPI 3 version required <a href="#31-asyncapi-300-required" id="31-asyncapi-300-required"></a>
...
**[M]** The file **MUST** pass a parser/validator for its declared AsyncAPI 3 version (for example, `@asyncapi/parser` or the AsyncAPI CLI).

## 3.5 Complete AsyncAPI info block <a href="#35-complete-asyncapi-info-block" id="35-complete-asyncapi-info-block"></a>

**[M]** The `info` block of each canonical AsyncAPI file **MUST** include `title`, `version` (SemVer), and a useful `description`.

It **SHOULD** include `contact`; repository governance may supply the maintainer contact when it does not belong in the API contract.
...
**[M+R]** The file **MUST** declare non-empty `servers`, `channels`, `operations`, and `components.messages`. A document with only schemas and no operations is not an API contract. AsyncAPI `servers` **MUST** describe the intended broker or transport endpoint pattern using AsyncAPI 3 server fields (`host`, `protocol`, optional `pathname`, variables, security, and protocol bindings). Server definitions **MUST NOT** point to `localhost`, personal developer machines, undocumented placeholders, or fake production brokers. Reserved documentation domains **MUST** be labelled as non-production.

Reference specifications that are not tied to a live broker **SHOULD** use parameterised server hosts and variables (for example, `host: "{brokerHost}"` with `protocol: mqtt`, `protocol: amqp`, `protocol: kafka`, or `protocol: wss`). Reserved documentation domains **MAY** be used only as variable defaults or examples.
...
**[M]** A BB that documents GovStack domain events **MUST** use a CloudEvents-conforming envelope as specified in [§17.6](../part-d/17-asyncapi-channel-rules.md#176-structured-cloudevents-json-payloads)–[§17.7](../part-d/17-asyncapi-channel-rules.md#177-shared-cloudevents-envelope-schema). A BB that documents asynchronous rejections **MUST** reference `GovStackAsyncError`, and **MUST** reuse `AsyncFieldError` when it exposes field-level errors. When shared components are used, `govstack-asyncapi-common.yaml` **MUST** be vendored locally at `api/common/govstack-asyncapi-common.yaml`, and the pinned version **MUST** be explicit. BB specifications own their Message Objects, security schemes, headers, examples, and protocol bindings because those objects require BB- and transport-specific values.

The shared `CloudEventEnvelope` **SHOULD** be reused when its event-type constraints fit the BB contract. A BB **MAY** define a local CloudEvents-conforming envelope for another stable event-type namespace permitted by [§16.3](../part-d/16-cloudevents-and-webhooks.md#163-reverse-dns-event-types).

## 3.9 JSON Schema payload conventions <a href="#39-json-schema-payload-conventions" id="39-json-schema-payload-conventions"></a>

**[M]** AsyncAPI documents **MUST** use JSON Schema compatible with their declared AsyncAPI 3 version for payload schemas and **MUST** follow the JSON conventions in [§9](../part-c/9-json-conventions-and-naming.md) and [§10](../part-c/10-data-types-and-formats.md) for GovStack-owned payload fields.
